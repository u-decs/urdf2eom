...
sym_I = sym('I_%d',[3,3,smds.NB],'real');
sym_m = sym('m_',[1,smds.NB],'real');
sym_pj = sym('pj',[3,smds.NB],'real');
sym_pm = sym('pm',[3, smds.NB],'real');
%%
%Generation loop
for i = 1:smds.NB
    % Parents
    ...
    smds.I(:,:,i) = mcI(sym_m(i),sym_pm(:,i),sym_I(:,:,i));
    
    %Transform tree
    if smds.parent(i) == 0
        angle = str2sym(split(model.robot.joint{i}.origin.Attributes.rpy));
        disp =  sym_pj(:,i);
    else
        angle = str2sym(split(model.robot.joint{i}.origin.Attributes.rpy))...
            -str2sym(split(model.robot.joint{i-1}.origin.Attributes.rpy));
        disp = sym_pj(:,i);
    end
